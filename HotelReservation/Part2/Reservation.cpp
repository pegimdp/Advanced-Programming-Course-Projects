...
Reservation::Reservation(int id_, std::string hotel_id_, std::string room_type_, int room_price_
...
void Reservation::print_reservation(void)
{
  std::cout << ID << COLON << SPACE << id << SPACE << HOTEL << COLON << SPACE << hotel_id << SPACE << ROOM << COLON << SPACE << room_type
            << SPACE << QUANTITY << COLON << SPACE << quantity << SPACE << COST << COLON << SPACE << cost << SPACE << CHECK_IN << COLON << SPACE
            << check_in << SPACE << CHECK_OUT << COLON << SPACE << check_out << SPACE << std::endl;
}
