...
DefaultFilter::DefaultFilter(hotels_t given_hotels)
{
  hotels = given_hotels;
}

void DefaultFilter::add_filter(float sample_mean, float sample_standard_deviation)
{
  hotels_t new_hotels = {};
  for(auto hotel:hotels)
  {
    if(abs(hotel->average_room_price - sample_mean) <= TWO * sample_standard_deviation)
      new_hotels.push_back(hotel);
  }
  hotels = new_hotels;
}

hotels_t DefaultFilter::get_hotels(void)
{
  return hotels;
}
